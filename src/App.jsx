...
const simulateStep = () => {
  ...
  // --- Termination logic ---
  // If all processes are not 'running' or 'waiting' (i.e., all are 'aborted' or have no resources/needs), stop simulation
  const allDone = processesCopy.every(
    (p) =>
      p.status !== 'running' ||
      (p.resources.length === 0 && p.needsResource === null)
  );
  if (allDone) {
    setIsRunning(false);
...
const resetSimulation = () => {
  ...
  setWaitForGraph([]);
  setAiLogs([]);
  // Re-initialize based on user input
  setProcesses(
  ...
  setResources(
    Array.from({ length: resourceCount }, (_, i) => ({
      id: `R${i + 1}`,
      instances: resourceInstances[i] || 1,
      allocatedTo: [],
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 font-sans">
      <div className="max-w-xl mx-auto bg-white rounded-lg shadow-lg p-8 mt-10">
        <h1 className="text-2xl font-bold mb-2">Deadlock Simulator Setup</h1>
        <p className="mb-6 text-gray-600">Enter the number of processes and resources to simulate. You can also set the number of instances for each resource.</p>
        <form
          onSubmit={e => {
            e.preventDefault();
            // Initialize processes and resources based on user input
            setProcesses(
              Array.from({ length: processCount }, (_, i) => ({
                id: `P${i + 1}`,
                color: processColors[i % processColors.length],
        ...
        >
          <div className="mb-4">
            <label className="block font-medium mb-1">Number of Processes</label>
            <input
              type="number"
              min={1}
              max={8}
              ...
              value={processCount}
              onChange={e => {
                setProcessCount(Number(e.target.value));
          ...
          <div className="mb-4">
            <label className="block font-medium mb-1">Number of Resources</label>
            <input
              type="number"
              min={1}
              max={8}
          ...
          <div className="mb-4">
            <label className="block font-medium mb-1">Instances per Resource</label>
            {Array.from({ length: resourceCount }, (_, i) => (
              <div key={i} className="flex items-center mb-2">
                <span className="w-20">Resource R{i + 1}:</span>
                <input
                  type="number"
                  min={1}
                  max={10}
                  className="border rounded p-1 w-24 ml-2"
                  value={resourceInstances[i] || 1}
                  onChange={e => {
                    const arr = [...resourceInstances];
                    arr[i] = Number(e.target.value);
                    setResourceInstances(arr);
                  }}
                  required
                />
              </div>
